...
def get_gender(df, key):
    ...
    df['executive1_gender'] = df.apply(
        lambda row: asyncio.run(extract_gender_fromname(row['executivecontact1_name'], key)), axis=1)
...
async def get_minor_detect(df):
    ...
    diverse_df1 = pred_wiki_name(df, lname_col='executivecontact1_lname', fname_col='executivecontact1_fname',
                                 num_iter=50)
    logger.info(f"<<<-- minority prediction for executive1 completed ------->>>")

    diverse_df2 = pred_wiki_name(df, lname_col='executivecontact2_lname', fname_col='executivecontact2_fname',
                                 num_iter=50)
    ...
    diverse_df[["executive1_race", "executive2_race"]] = diverse_df[["executive1_race", "executive2_race"]].replace(
        to_replace=["British", "French", 'Italian', 'Germanic', 'Nordic', 'EastEuropean', 'Jewish'], value='')
...
def get_region(df, config):
    ...
    df['executive1_origin'] = df.apply(
        lambda row: asyncio.run(extract_origin_fromname(row['executivecontact1_name'], config.get("region_key"))),
        axis=1)
    ...
    df['executive2_origin'] = df.apply(
        lambda row: asyncio.run(extract_origin_fromname(row['executivecontact2_name'], config.get("region_key"))),
        axis=1)
    ...
    origin_df[["executive1_origin", "executive2_origin"]] = origin_df[
        ["executive1_origin", "executive2_origin"]].replace(to_replace=str(config.get("region_list")).split(","),
                                                            value='')
    origin_df['executive2_origin'].mask(origin_df['executive2_origin'].isnull(), '', inplace=True)
    origin_df["origin"] = origin_df[['executive1_origin', 'executive2_origin']].apply(
        lambda x: ','.join(x.replace(np.nan, '')), axis=1)
    logger.info("<<<---- Gender prediction is complete ---->>>")
    return origin_df
